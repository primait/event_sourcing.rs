...
type Agg = AggregateManager<CounterAggregate>;
...
pub async fn increment_atomically(manager: Agg, aggregate_id: Uuid) -> Result<(), CounterError> {
...
pub async fn increment_optimistically(manager: Agg, aggregate_id: Uuid) -> Result<(), CounterError> {
...
pub async fn with_atomic_read(manager: Agg, aggregate_id: Uuid) -> Result<(), CounterError> {
    let aggregate_state = manager.lock_and_load(aggregate_id).await?.unwrap_or_default();
...
pub async fn with_optimistic_read(manager: Agg, aggregate_id: Uuid) -> Result<(), CounterError> {
