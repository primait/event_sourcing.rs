...
use crate::aggregate::{FooAggregate, FooCommand, FooError};
...
pub async fn increment_atomically(aggregate: FooAggregate, aggregate_id: Uuid) -> Result<(), FooError> {
    let aggregate_state = aggregate.lock_and_load(aggregate_id).await?.unwrap_or_default();
    aggregate.handle_command(aggregate_state, FooCommand).await?;
    Ok(())
}
...
pub async fn increment_optimistically(aggregate: FooAggregate, aggregate_id: Uuid) -> Result<(), FooError> {
...
pub async fn with_atomic_read(aggregate: FooAggregate, aggregate_id: Uuid) -> Result<(), FooError> {
    let aggregate_state = aggregate.lock_and_load(aggregate_id).await?.unwrap_or_default();
...
pub async fn with_optimistic_read(aggregate: FooAggregate, aggregate_id: Uuid) -> Result<(), FooError> {
