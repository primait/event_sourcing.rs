...
use crate::{aggregate::CounterAggregate, structs::CounterCommand, projector::Counter};
...
async fn main() {
    ...
    let () = sqlx::migrate!("./migrations").run(&pool).await.expect("Failed to run migrations");

    let count_id = Uuid::new_v4();

    // Construct the aggregation, and some nil state for it
    let aggregate = CounterAggregate::new(&pool).await.expect("Failed to construct aggregate");
    ...
    sqlx::query("DROP TABLE counters").execute(&pool).await.expect("Failed to drop table");
    sqlx::query("CREATE TABLE counters (\"counter_id\" UUID PRIMARY KEY NOT NULL, \"count\" INTEGER NOT NULL );")
        .execute(&pool).await.expect("Failed to recreate counters table");
    ...
    {
        let events = aggregate.event_store().by_aggregate_id(count_id).await.expect("Failed to retrieve events");
        let projector = projector::CounterProjector{};
        let mut connection = pool.acquire().await.expect("Failed to acquire connection");
        let _ = sqlx::query("BEGIN").execute(&mut connection).await.expect("Failed to begin transaction");
        for event in events {
            projector.project(&event, &mut connection).await.expect("Failed to project event");
        }
        let _ = sqlx::query("COMMIT").execute(&mut connection).await.expect("Failed to commit rebuild");
    }

    // Assert the counter has been rebuilt
    let res = Counter::by_id(count_id, &pool).await.expect("Query failed").expect("counter not found");
