...
mod tests {
    ...
    use esrs::{StoreParams, Identifiable};
    ...
    async fn check_amounts() {
        ...
        let result = payment_aggregate
            .handle_command(state.clone(), PaymentCommand::Pay { amount: -10.0 })
            .await;
        ...
        let result = payment_aggregate
            .handle_command(state.clone(), PaymentCommand::Refund { amount: -10.0 })
            .await;
        ...
        let state: AggregateState<PaymentState> = payment_aggregate
            .handle_command(state, PaymentCommand::Refund { amount: 10.0 })
            .await
            .unwrap();
