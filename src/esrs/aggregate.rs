...
pub trait AggregateManager: Aggregate {
    ...
    ///
    /// You should _avoid_ implementing this function, and be _very_ careful if you decide to do so.
    async fn load(&self, aggregate_id: impl Into<Uuid> + Send + 'async_trait) -> Option<AggregateState<Self::State>> {
    ...
    async fn delete(&self, aggregate_id: impl Into<Uuid> + Send + 'async_trait) -> Result<(), Self::Error> {
